...
export async function listUpcomingBookings({
  ...
  if (decodedCursor) {
    ...
    cursorClause =
      "AND (bookings.start_at, bookings.id) > ($3::timestamptz, $4::uuid)";
  }
  values.push(BOOKING_PAGE_SIZE + 1);
  ...
  const hasMore = result.rows.length > BOOKING_PAGE_SIZE;
  const rows = result.rows.slice(0, BOOKING_PAGE_SIZE);
  return {
